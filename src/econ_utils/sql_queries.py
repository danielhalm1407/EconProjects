...
DATABASE_PATH = os.path.join(current_dir, '..', '..', 'data', 'econ.db')
def get_sql_engine(DATABASE_PATH = DATABASE_PATH):
    """
    ...
    if not hasattr(get_sql_engine, 'engine'):
        get_sql_engine.engine = create_engine('sqlite:///' + DATABASE_PATH)
